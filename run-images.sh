...
# Build Images ------------------------
sudo docker rmi $OS_TYPE:$VERSION-temp
sudo docker build . \
--build-arg OS_VERSION=22.04 \
--build-arg OS_SYSTEMS=ubuntu \
-t $OS_TYPE:$VERSION-temp

# RUN Images --------------------------
sudo docker run -itd --privileged \
--name $D_NAMES \
-h $D_NAMES-cd1.pika.wiki \
$PORTMAP \
-p $PM_SSHS:22 \
$OS_TYPE:$VERSION-temp
# Message -----------------------------
PORTMAP=${PORTMAP//-\\n\\n\\n}
PORTMAP=${PORTMAP//-p/\\n\\t}
...
echo    ─────────────────────────────────────────────────────────────────────
echo                    Docker Container Create Successfully                  
echo        
echo                       Container $D_NAMES                                                                 
echo                       OSSystem: $OS_TYPE $VERSION                            
echo                       SSH Port: v4-cd1.pika.wiki:$PM_SSHS
echo                       Username: root         
...
echo -e                       $PORTMAP    
echo    ─────────────────────────────────────────────────────────────────────     
echo Password: $D_NAMES $D_PASSW >> ~/docker-user.conf
